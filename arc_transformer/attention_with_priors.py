...
class RelativeMultiheadAttention(torch.nn.Module):
    def __init__(self,
        ...
        self.query_transform = torch.nn.Linear(in_features=embedding_dim, out_features=embedding_dim,
                                               bias=bias)
        self.key_content_transform = torch.nn.Linear(in_features=embedding_dim, out_features=embedding_dim,
                                             bias=bias)
        self.key_location_transform = torch.nn.Linear(in_features=embedding_dim, out_features=embedding_dim,
                                             bias=bias)
        self.value_transform = torch.nn.Linear(in_features=embedding_dim, out_features=embedding_dim,
                                               bias=bias)
        self.output_projection = torch.nn.Linear(in_features=embedding_dim, out_features=embedding_dim,
                                                 bias=bias)
        self.register_buffer('query_content_bias', torch.rand(1, self.embedding_dim))
        self.register_buffer('query_location_bias', torch.rand(1, self.embedding_dim))

        self.dropout = torch.nn.Dropout(dropout)
        self.logit_scaling = 1 / math.sqrt(embedding_dim / self.num_heads)

        self._reset_parameters()

    def forward(self, query_source, key_source, value_source=None):
        ...
        # attention_regions: target_seq x batch (attention is restricted to regions with the same value)
        ...
        # positional features
        diagonal_index = 0 if self.causal else t
        relative_positions = torch.arange(0 if self.anticausal else -t, (0 if self.causal else t) + 1).float()  # r
        relative_position_features = self.positional_encoding(relative_positions.to(query_source.device))  # r x e
        ...
        location_key = self.key_location_transform(relative_position_features).view(-1, 1, h, eh)  # r x 1 x h x eh
        ...
        skewed_location_logits = self.skew(unskewed_location_logits, diagonal_index=diagonal_index)  # n*h x t x s

        content_bias = torch.bmm(self.query_content_bias.repeat(n, 1).view(n*h, 1, eh), key)  # n*h x 1 x s
        unskewed_location_bias = torch.bmm(self.query_location_bias.repeat(n, 1).view(n*h, 1, eh), location_key)  # n*h x 1 x r
        skewed_location_bias = self.skew(unskewed_location_bias.repeat(1, t, 1), diagonal_index=diagonal_index)  # n*h x t x s

        logits = content_based_logits + skewed_location_logits + content_bias + skewed_location_bias  # n*h x t x s

        mask = torch.zeros(t, t).to(key.device)
        minus_inf = torch.full((t, t), fill_value=float('-inf')).to(key.device)

        mask = mask[None].repeat(n, 1, 1)  # n x t x s

        mask = mask[:, None].repeat(1, h, 1, 1).view(n*h, t, s)
        logits += mask
    ...
    @staticmethod
    def skew2d(unskewed_logits, diagonal_index, h, w):
        '''
        :param unskewed_logits: n x t x r   n: batch, t: target dimension (query), r: relative position
        :param diagonal_index: index of the zero position in the r dimension (the relative position that will be on the
                         main diagonal)
        ...
        dev = unskewed_logits.device
        n, t, r = unskewed_logits.shape
        num_past_steps = (r - diagonal_index)
        num_future_steps = diagonal_index
        out_steps = max(num_future_steps, num_past_steps)
